...
cron.schedule('* * * * *', async () => {
    try {
        const [rows] = await dbPool.query(`SELECT id, upload_path, optimized_path 
             FROM sessions 
        ...
        for (const row of rows) {
            ...
            try {
                ...
                await dbPool.query(`UPDATE sessions SET status = 'abandoned' WHERE id = ?`, [row.id]);
    ...
    try {
        ...
        for (const row of rows) {
            try {
                ...
                await dbPool.query(`DELETE FROM sessions WHERE id = ?`, [row.id]);
