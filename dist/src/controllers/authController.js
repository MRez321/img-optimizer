...
import { findUserByEmail, findUserById, createUser, updateUserProfile, markEmailVerified, findOAuthAccount, linkOAuthAccount, storeRefreshToken, findRefreshToken, verifyRefreshTokenHash, revokeRefreshToken, revokeAllRefreshTokensForUser, createEmailVerificationCode, findValidVerificationCode, deleteVerificationCodesForUser, toPublicUser, } from '../models/userModel.js';
import { signAccessToken, signRefreshToken, verifyRefreshToken, REFRESH_TOKEN_TTL_MS, ACCESS_COOKIE_NAME, REFRESH_COOKIE_NAME, accessCookieOptions, refreshCookieOptions, } from '../config/jwt.js';
import { sendVerificationEmail } from '../config/mailer.js';
const GOOGLE_CLIENT_ID = process.env.GOOGLE_CLIENT_ID || '';
const googleClient = new OAuth2Client(GOOGLE_CLIENT_ID);
const BCRYPT_ROUNDS = 10;
// Issues a fresh access + refresh token pair for a user and sets cookies.
const issueTokens = async (res, userId, email, role) => {
...
export const register = async (req, res) => {
    ...
    sendVerificationEmail(user.email, code).catch((err) => console.error('sendVerificationEmail failed:', err));
